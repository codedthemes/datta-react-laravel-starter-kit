// react-bootstrap
import Col from 'react-bootstrap/Col';
import ProgressBar from 'react-bootstrap/ProgressBar';
import Row from 'react-bootstrap/Row';

// project-imports
import MainCard from 'components/MainCard';

const ratings = [
  { stars: 5, count: 384, progress: 70 },
  { stars: 4, count: 145, progress: 35 },
  { stars: 3, count: 24, progress: 25 },
  { stars: 2, count: 1, progress: 10 },
  { stars: 1, count: 0, progress: 0 }
];

// ==============================|| WIDGETS DATA - RATING ||============================== //

export default function WidgetsDataRating() {
  return (
    <MainCard title="Rating" bodyClassName="pb-0">
      <Row className="align-items-center justify-content-center mb-3">
        <Col xs={6}>
          <h2 className="fw-bold d-flex align-items-center float-start m-0">
            4.7
            <i className="fas fa-star f-10 ms-2 text-warning" />
          </h2>
        </Col>
        <Col xs={6}>
          <h6 className="d-flex align-items-center float-end m-0">
            0.4
            <i className="fas fa-caret-up text-success ms-2 f-20" />
          </h6>
        </Col>
      </Row>
      {ratings.map((rating) => (
        <Row key={rating.stars}>
          <Col xl={12}>
            <h6 className="align-items-center float-start">
              <i className="fas fa-star f-10 m-r-10 text-warning" />
              {rating.stars}
            </h6>
            <h6 className="align-items-center float-end">{rating.count}</h6>
            <div className="m-t-30 m-b-20">
              <ProgressBar variant="brand-color-1" now={rating.progress} />
            </div>
          </Col>
        </Row>
      ))}
    </MainCard>
  );
}
