    ...
    <Stack direction="horizontal" gap={2} className="align-items-center justify-content-center">
      {meta?.selectedRow[row.id] && (
        <OverlayTrigger placement="bottom" overlay={<Tooltip>Cancel</Tooltip>}>
          <i className="ti ti-plus rotate-45 text-danger f-22" onClick={(e) => setSelectedRow(e as React.MouseEvent<HTMLButtonElement>)} />
        </OverlayTrigger>
      )}
      <OverlayTrigger placement="bottom" overlay={<Tooltip>{meta?.selectedRow[row.id] ? 'Save' : 'Edit'}</Tooltip>}>
        <i
          className={`${meta?.selectedRow[row.id] ? 'ti ti-send text-success' : 'ti ti-pencil text-primary'} f-22`}
          onClick={(e) => setSelectedRow(e as React.MouseEvent<HTMLButtonElement>)}
        />
      </OverlayTrigger>
    </Stack>
  );
...
export default function UmbrellaTable() {
  const columns = useMemo<ColumnDef<any>[]>(
    () => [
      ...
      },
      {
        ...
        enableGrouping: false,
        cell: (cell) => (
          <Image src={getImageUrl(`avatar-${cell.getValue()}.jpg`, ImagePath.USER)} alt="User Avatar" className="avatar avatar-xs" />
        )
      },
      {
        id: 'fullName',
  ...
  return (
    <MainCard>
      <Stack direction="horizontal" gap={2} className="align-items-center justify-content-between p-3">
        <DebouncedInput value={globalFilter ?? ''} onFilterChange={(value) => setGlobalFilter(String(value))} />
        <SortingData getState={table.getState} setPageSize={table.setPageSize} />
        <Stack direction="horizontal" gap={2} className="align-items-center w-100 w-sm-auto justify-content-end">
          <Form.Select
            value={statusFilter}
            className="w-200"
              ...
              <tr key={headerGroup.id}>
                {headerGroup.headers.map((header, index) => (
                  <td
                    ...
                    className={`${(header.column.columnDef.meta as ColumnMeta)?.className} ${index === 0 ? 'ps-24' : ''}
                          ${index === headerGroup.headers.length - 1 ? 'pe-24' : ''}`}
                  >
