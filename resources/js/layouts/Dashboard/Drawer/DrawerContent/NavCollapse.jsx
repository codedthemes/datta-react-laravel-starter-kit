  ...
  if (isHorizontal) {
    ...
    return (
      <ListGroup
        ...
        <ul
          ref={submenuRef}
          className={`pc-submenu ${(themeDirection === ThemeDirection.RTL || isEdge) && 'edge'}`}>
          {navCollapse}
        </ul>
      </ListGroup>
    );
  ...
  return (
    <ListGroup className={`pc-item pc-hasmenu ${open ? 'pc-trigger' : ''}`}>
      ...
      <Collapse in={open} mountOnEnter unmountOnExit>
        <div>
          <ul
            className={`pc-submenu ${themeDirection === ThemeDirection.RTL && 'edge'}`}>{navCollapse}</ul>
        </div>
      </Collapse>
    </ListGroup>
  );
...
NavCollapse.propTypes = {menu:PropTypes.any,
